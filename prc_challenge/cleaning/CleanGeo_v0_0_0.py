...
class CleanGeo_v0_0_0(BaseCleaning):
    ...
    def __call__(
        ...
        for flight_id in tqdm(Flight.flight_ids):
            ...
            if file_path.exists():
                continue
            traj = Flight[flight_id][['altitude','latitude','longitude','mach','groundspeed','vertical_rate','TAS', 'timestamp']]
            traj['timestamp'] = pd.to_datetime(traj['timestamp'])
            condition = (traj['altitude'] >= 0) & (traj['altitude'] <= 60000) & (traj['latitude'].between(-90, 90)) & (traj['longitude'].between(-180, 180))
            traj = traj[condition]
            traj['time_bin'] = (traj['timestamp'].astype('int64') // 5_000_000_000)  # 5 seconds in nanoseconds
            traj = traj.groupby('time_bin').agg({
                'altitude': 'mean',
                'latitude': 'mean',
                'longitude': 'mean',
                'mach': 'mean',
