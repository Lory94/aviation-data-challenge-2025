...
class AddGeoInfo_v0_0_1(BaseFeatureEngineering):
    ...
    def __call__(
        ...
        FuelSegment_X.to_csv(Path(self.directory).expanduser() / "features_v0_0_1.csv")
    ...
    def _get_geo_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "range_altitude[ft]" in self.method:
            agg_dict["range_altitude[ft]"] = (
                "altitude",
                lambda x: x.max() - x.min() if len(x) > 1 else 0.0,
            )
        if "delta_altitude[ft]" in self.method:
            agg_dict["delta_altitude[ft]"] = (
                "altitude",
                lambda x: x.iloc[-1] - x.iloc[0] if len(x) > 1 else 0.0,
            )
