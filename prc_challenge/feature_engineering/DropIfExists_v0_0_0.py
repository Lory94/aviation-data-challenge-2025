...
class DropIfExists_v0_0_0(BaseFeatureEngineering):
    ...
    def __call__(
        ...
        if len(non_present_columns) > 0:
            print(
                f"[DropIfExists] Attempted to drop columns {non_present_columns}, but they are not present."
            )

        FuelSegment_X = FuelSegment_X.drop(
            columns=[col for col in FuelSegment_X if col in self.drop_columns]
        )
        ...
        return FuelSegment_X, new_column_functions
